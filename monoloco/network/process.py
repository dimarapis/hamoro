...
def preprocess_monoloco(keypoints, kk, zero_center=False):
    ...
    if isinstance(keypoints, list):
        keypoints = keypoints.clone().detach().to(dtype=torch.float32)
        #keypoints = torch.tensor(keypoints).to(dtype=torch.float32)
...
def extract_outputs(outputs, tasks=()):
    ...
    if outputs.shape[1] == 2: 
        ...
        dic_out = {'d' :outputs[:, 0:1],
                'uncertainty' : outputs[:, 1:2]}
        return dic_out
